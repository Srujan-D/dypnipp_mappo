...
from .FireCommander_Utils import EnvUtilities
...
class FireCommanderExtreme(object):
    def __init__(
        ...
        if start is not None:
            ...
            for _ in range(self.perception_agent_num + self.action_agent_num):
                self.start.append([
    ...
    def env_init(self, comm_range=30, init_alt=10):
        ...
        for i in range(self.perception_agent_num):
            ...
            self.agent_state.append(
                [int(self.start[i][0]), int(self.start[i][1]), init_alt, 0]
            )
    ...
    def single_agent_state_update(self, loc, agent_id=0):
    ...
    #     # Fast initial population of fire intensities
    ...
    #     # Pre-compute indices of fire locations
    #     fire_x, fire_y = np.nonzero(field_intensity)
    #     fire_intensities = field_intensity[fire_x, fire_y]
    #     num_fire_points = len(fire_x)

    #     # Use vectorized operations for faster neighbor computation
    #     for i in range(world_size):
    #         for j in range(world_size):
    #             if field_intensity[i, j] == 0:
    #                 # Compute distances to all fire points at once
    #                 distances = (fire_x - i) ** 2 + (fire_y - j) ** 2
    #                 # Find nearby points (distance <= 2)  
    #                 nearby_mask = distances <= 2
    #                 nearby_intensities = fire_intensities[nearby_mask]
                    
    #                 if len(nearby_intensities) > 0:
    #                     field_intensity[i, j] = nearby_intensities.mean()
