...
class SeparatedReplayBuffer:
    def __init__(self, args, obs_shape, share_obs_shape, act_space, agent_ID):
        ...
        self.masks = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads,  self.sample_size + 2, self.k_size), dtype=np.float32
        )
        self.actions = np.zeros(
            (self.episode_length, self.n_rollout_threads, act_space["n"]),
            dtype=np.float32,
        )
        self.action_log_probs = np.zeros_like(self.actions)
    ...
    def insert(
        ...
        self.node_inputs[self.step + 1] = node_inputs.copy()
        self.edge_inputs[self.step + 1] = edge_inputs.copy()
        self.budget_inputs[self.step + 1] = budget_inputs.copy()
        self.current_index[self.step + 1] = current_index.copy()
        self.pos_encoding[self.step + 1] = pos_encoding.copy()
    ...
    def feed_forward_generator(
        ...
        rnn_states_actor = self.rnn_states_actor[:-1].reshape(-1, *self.rnn_states_actor.shape[2:])
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                rnn_states_batch.append(rnn_states_actor[ind])
